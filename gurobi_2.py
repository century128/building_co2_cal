...
def plan_carbon_problem(ce_an,p_d,g_d,g_w,q_d,i_pv,fai_wt,period):
    ...
    g_bio_k = [m.addVar(name=f"g_bio_k{i}") for i in range(period)]
    g_bio_out_k = [m.addVar(name=f"g_bio_out_k{i}") for i in range(period)]
    ...
    for i in range(period):
        m.addConstr(p_pv_k[i] == i_pv[i] * s_pv * pv_para[0])
        m.addConstr(g_hyout_k[i] == hy_para[0] * m_hyout_k[i])
        m.addConstr(p_wtg_k[i] == fai_wt[i] * p_wtg)
        m.addConstr(g_stc_k[i] == i_pv[i] * s_stc * stc_para[0])
        m.addConstr(g_gshp_k[i] == gshp_para[0] * p_gshpG_k[i] * gshp_para[2])
        m.addConstr(q_gshp_k[i] == gshp_para[1] * p_gshpQ_k[i] * gshp_para[2])
        m.addConstr(g_bhp_gshp_k[i] == g_gshp_k[i] * bhp_para[0])
        m.addConstr(g_gshp_bhp_k[i] == g_gshp_k[i] * bhp_para[1])
        m.addConstr(g_bio_k[i] == m_bio_k[i] * bio_para[0])
        m.addConstr(g_bio_out_k[i] == g_bio_k[i] * bio_para[1])
        m.addConstr(g_ashp_k[i] == p_ashpG_k[i] * ashp_para[0] * ashp_para[2])
        m.addConstr(q_ashp_k[i] == p_ashpQ_k[i] * ashp_para[1] * ashp_para[2])
        m.addConstr(g_eb_k[i] == p_eb_k[i] * eb_para[0] *eb_para[1])
        m.addConstr(v_gfb_k[i] == g_gfb_k[i] / gfb_para[0])
        m.addConstr(m_cfb_k[i] == g_cfb_k[i] / cfb_para[0])
        m.addConstr(v_ofb_k[i] == g_ofb_k[i] / ofb_para[0])
        m.addConstr(q_ru_k[i] == p_ru_k[i] * ru_para[0])
        ...
        m.addConstr(p_gshpQ_k[i]/gshp_para[2] + p_gshpG_k[i]/gshp_para[2] <= p_gshp)
    ...
    # Set objective
    ## set CE_total
    CE_ashp = m.addVar(name="CE_ashp")
    CE_stc = m.addVar(name='CE_stc')
    CE_bb = m.addVar(name='CE_bb')
    CE_eb = m.addVar(name='CE_eb')
    CE_pv = m.addVar(name='CE_pv')
    CE_cfb = m.addVar(name='CE_cfb')
    CE_gfb = m.addVar(name='CE_gfb')
    CE_ofb = m.addVar(name='CE_ofb')
    CE_gshp = m.addVar(name='CE_gshp')
    CE_wtg = m.addVar(name='CE_wtg')
    CE_hy = m.addVar(name='CE_hy')
    CE_wt = m.addVar(name='CE_wt')
    CE_ct = m.addVar(name='CE_ct')
    CE_ru = m.addVar(name='CE_ru')
    CE_r = m.addVar(name='CE_r')
    ce_es = m.addVar(name='ce_es')
    ce_cpesr = m.addVar(name='ce_cpesr')
    ce_total = m.addVar(name='ce_total')
    c_invest = m.addVar(name='c_invest')
    c_om1 = m.addVar(name='c_om1')
    c_om = m.addVar(name='c_om')
    c_re = m.addVar(name='c_re')
    zeta = m.addVar(name='zeta')
    ...
    ## set zeta
    # zeta = 0.1*300*s_pv
    m.addConstr(CE_ashp == p_ashp * 2.83 / 15 * 2.88)
    m.addConstr(CE_stc == 25.2 * 10 ** (-6) * s_stc)
    m.addConstr(CE_bb == 1788 * 10 ** (-6) * m_bio)
    m.addConstr(CE_eb == 650 * 10 ** (-6) * p_eb)
    m.addConstr(CE_pv == 34 * 10 ** (-6) * s_pv)
    m.addConstr(CE_cfb == 876 * 10 ** (-6) * g_cfb)
    m.addConstr(CE_gfb == 412 * 10 ** (-6) * g_gfb)
    m.addConstr(CE_ofb == 579 * 10 ** (-6) * g_ofb)
    m.addConstr(CE_gshp == 79 * 10 ** (-6) * p_gshp)
    m.addConstr(CE_wtg == 17 * 10 ** (-6) * p_wtg)
    m.addConstr(CE_hy == 51 * 10 ** (-6) * p_fc)
    m.addConstr(CE_wt == 500 * 10 ** (-6) * v_wt)
    m.addConstr(CE_ct == 500 * 10 ** (-6) * v_ct)
    m.addConstr(CE_ru == 2000 * 10 ** (-6) * p_ru)
    m.addConstr(CE_r == CE_ashp + CE_ru + CE_gshp)
    m.addConstr(ce_es == CE_ashp + CE_stc + CE_bb + CE_eb + CE_pv + CE_cfb + CE_gfb + CE_ofb + CE_gshp + CE_wtg + CE_hy + CE_wt + CE_ct + CE_ru + CE_r)
    m.addConstr(ce_cpesr == gp.quicksum([p_g_k[i]*0.6101 + m_cfb_k[i]*8.14 + v_gfb_k[i]*1.535 + v_ofb_k[i]*2.25 for i in range(period)]))
    m.addConstr(ce_total == ce_an + ce_es + ce_cpesr)
    m.addConstr(c_invest == 3000*p_ashp + 1500*s_stc + 360*m_bio + 160*p_eb + 6600*s_pv + 3980*g_gfb + 3980*g_ofb + 3980*g_cfb + 400*p_gshp + 3600*p_wtg + 10000*p_fc + 10000*p_el + 1000*p_ru + 3000*m_hy + 500*v_wt +500*v_ct)
    m.addConstr(c_om1 == gp.quicksum([p_g_k[i]*0.58 + m_bio_k[i]*0.8 + m_cfb_k[i]*1.2 + 3.5*v_gfb_k[i] + 3.75*v_ofb_k[i] + 40*m_hy_purIn_k[i] for i in range(period)]))
    m.addConstr(c_om == c_om1+ 0.02*c_invest)
    m.addConstr(c_re == c_invest*0.02)
    m.addConstr(zeta == 0.1*300*s_pv)

    m.setObjective(0.06*ce_total + CRF*m_crf*c_invest + c_om - c_re - zeta,GRB.MINIMIZE)

    try:
        m.optimize()
        print('Obj: %g' % m.ObjVal)
        # print('p_ashp:%s' % p_ashp.X)
        # print('s_stc:%s' %s_stc.X)
        # print('m_bio: %s' %m_bio.X)
        name = ['minimize']
        value = [m.ObjVal]
        for v in m.getVars():
            # print('%s %g' % (v.VarName, v.X))
            name.append(v.VarName)
            value.append(v.X)
        output_csv_dic = {'name':name,'value':value}
        output_csv = pd.DataFrame(output_csv_dic)
